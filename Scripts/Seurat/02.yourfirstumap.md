...
pbmc.genes <- wilcoxauc(Innateliketcells, 'subset')
cluster_genes <- pbmc.genes %>% filter(group == as.character("PILT0"))  %>% filter(padj <1) %>% arrange(desc(logFC)) %>% dplyr::select(feature, logFC)
ranks <- deframe(cluster_genes)
em2 <- clusterProfiler::GSEA(ranks, TERM2GENE = all_pathways_tbl, eps = 1e-300, minGSSize = 20, pvalueCutoff = 0.05, nPermSimple=10000, seed = 44)
plot <- clusterProfiler::dotplot(em2, showCategory = 10, font.size= 5,color = "p.adjust") + facet_grid(. ~ .sign)
```
